...
class CommandDispatcher:
    ...
    async def handle_command(self, event):
        ...
        logging.debug("Kiruvchi buyruq!")

        if event.sticker or event.dice or event.audio:
            logging.debug("Ko'rinmas yoki potentsial yuborilgan buyruqni e'tiborsiz qoldirish.")
            return

        if event.via_bot_id:
            logging.debug("Inline botni e'tiborsiz qoldirish.")
            return
        ...
        if utils.get_chat_id(message) in blacklist_chats or (
            ...
            logging.debug("Xabar qora ro'yxatga kiritilgan")
            return
        ...
        if len(tag) == 2:
            ...
            logging.debug("Laqabsiz xabarni e'tiborsiz qoldiring")
            return
        ...
        if func is not None:
            ...
            if message.is_channel and message.is_group:
                ...
                if (
                    chat.title.startswith(f"dirxam-{my_id}-")
                    or chat.title == "dirxam-log"
                ):
                    logging.warning("Ma'lumotlar bazasida xabarni e'tiborsiz qoldiring \\ logging chat")
                    return
            ...
            if (
                ...
                logging.debug("Buyruq chatda qora ro'yxatda")
                return
            ...
            if (
                ...
                logging.debug("Buyruq chatda oqilovchi emas")
                return
