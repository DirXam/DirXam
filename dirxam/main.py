...
def get_phones(arguments):
    ...
    phones = {
        ...
        for phone in map(
            lambda f: f[18:-8],
            filter(
                lambda f: f.startswith("dirxam-") and f.endswith(".session"),
...
def main():  # noqa: C901
    ...
    while api_token is None:
        ...
        if web:
            ...
            print("Veb-mode tayyor")  # noqa: T001
            port = str(web.port)
            if platform.system() == "Linux" and not os.path.exists(
                "/etc/os-release"
            ):
                print(f"Iltimos, tashrif buyuring http://localhost:{port}")
            else:
                ipaddress = get("https://api.ipify.org").text
                print(
                    f"Iltimos, tashrif buyuring http://{ipaddress}:{port} yoki http://localhost:{port}"
                )
        ...
        if web:
            ...
            for client in clients:
                session = SQLiteSession(
                    os.path.join(
                        arguments.data_root or BASE_DIR,
                        f"dirxam-+{'X' * (len(client.phone) - 5)}{client.phone[-4:]}", # skipcq: FLK-E501
                    )
                )
    ...
    for phone_id, phone in phones.items():
        session = os.path.join(
            arguments.data_root or BASE_DIR,
            f"dirxam{(('-' + phone_id) if phone_id else '')}",
        )
        ...
        try:
            ...
            print(
                ...
                f"'dirxam-{phone if phone else ''}.session'"
            )
            continue
...
╚═════╝░╚═╝╚═╝░░╚═╝╚═╝░░╚═╝╚═╝░░╚═╝╚═╝░░░░░╚═╝\\
                     ...
                     • {upd}
                     • Platforma: {_platform}
                     - Boshlandi {(await client.get_me(True)).user_id} -"""

            print(logo1)

            logging.info(f"=== BUILD: {build} ===")
            logging.info(
                f"=== VERSIYA: {'.'.join(list(map(str, list(__version__))))} ==="
            )
            logging.info(
                f"=== PLATFORMA: {utils.get_platform_name()} ==="
            )
        except Exception:
            logging.exception(
                "Notge xatosi"
            )  # Bu qism unchalik kerak emas, shuning uchun agar xato bo'lsa, uni e'tiborsiz qoldiring
