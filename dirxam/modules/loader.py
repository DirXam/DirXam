...
class LoaderMod(loader.Module):
    ...
    strings = {
        "name": "DirXam Saqlagich",
        "avail_header": "<b>🌇 DirXam\n</b>",
        ...
        "inline_init_failed": """🚫 <b>This module requires DirXamTG inline feature and initialization of InlineManager failed</b>
<i>Please, remove one of your old bots from @BotFather and restart userbot to load this module</i>""",
        "version_incompatible": "🚫 <b>This module requires DirXamTG {}+\nPlease, update with </b><code>.update</code>",
    ...
    def __init__(self):
        ...
        self.config = loader.ModuleConfig(
            "MODULES_REPO",
            "https://raw.githubusercontent.com/DirXam/DM-Modules/main/",
    ...
    async def load_module(
        ...
        if re.search(r"# ?scope: ?dirxamtg_min", doc):
            ver = re.search(r"# ?scope: ?dirxamtg_min ([0-9]+\.[0-9]+\.[0-9]+)", doc).group(1)
        ...
        module_name = "dirxam.modules." + uid
    ...
    async def unloadcmd(self, message: Message) -> None:
        ...
        for mod in worked:
            if not mod.startswith("dirxam.modules.") or not mod:
                raise Exception("Assertion error")

            without_prefix += [
                unescape_percent(mod[len("dirxam.modules.") :])
            ]
    ...
    async def clearcmd(self, message: Message) -> None:
        ...
        self._db.set("dirxam.modules.loader", "loaded_modules", [])
        self._db.set("dirxam.modules.loader", "unloaded_modules", [])
