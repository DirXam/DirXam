...
def get_git_info():
    ...
    return [
        ver,
        f"https://github.com/DirXam/DirXam/commit/{ver}"
        if ver
        else "",
    ]
...
async def get_user(message):
    ...
    if isinstance(message.peer_id, (PeerChannel, PeerChat)):
        ...
        logging.critical("user isn't in the group where they sent the message")
        return None

    logging.critical("`peer_id` is not a user, chat or channel")
    return None
        ...
        if len(txt) >= 4096:
            ...
            ret = [
                await message.client.send_file(
                    message.peer_id,
                    file,
                    caption="<b>📤 Buyruq chiqishi juda uzoq bo'lgan ko'rinadi, shuning uchun u faylda yuboriladi.</b>",  # noqa: E501
                ),
            ]
