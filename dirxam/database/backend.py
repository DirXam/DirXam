...
class CloudBackend:
    ...
    async def _find_asset_channel(self):
        async for dialog in self._client.iter_dialogs(None, ignore_migrated=True):
            if (
                dialog.name == f"dirxam-{self._me.user_id}-assets"
            ...
            return (
                await self._client(
                    CreateChannelRequest(
                        f"dirxam-{self._me.user_id}-assets",
                        "// Don't touch",
                        megagroup=True,
                    )
                )
    ...
    async def do_download(self):
        ...
        if not is_base_saved:
            chat = [
                ...
                if chat.name == f"dirxam-{self._me.user_id}-data" and chat.is_channel
            ]
            if not chat:
            ...
            async for msg in msgs:
                if isinstance(msg, Message):
                    ...
                    logger.debug(f"Topildi Xizmat xabari {msg}")
            await self.do_upload(data)
    ...
    async def do_upload(self, data):
        ...
        try:
            ...
            logger.exception("Ma'lumotlar bazasini saqlash muvaffaqiyatsiz tugadi!")
            raise
