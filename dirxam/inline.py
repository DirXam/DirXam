...
photo = io.BytesIO(
    requests.get(
        "https://github.com/DirXam/DirXam/raw/master/dirxam/bot_avatar.png" # noqa: E501, W505
    ).content
)
...
class InlineManager:
    def __init__(self, client, db, allmodules) -> None:
        ...
        self._token = db.get("dirxam.inline", "bot_token", False)
    ...
    async def _create_bot(self) -> None:
        ...
        # We create new bot
        logger.info("Foydalanuvchi Bot yo'q, yangisini yaratishga urinmoqda")
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            # Set its name to user's name + DirXam Userbot
            m = await conv.send_message(f"{self._name} ning DirXam userboti 🤖")
            r = await conv.get_response()
    ...
    async def _assert_token(
        ...
        logger.info("Bot tokeni db da topilmadi, Botf-da qidirishga urinish")
        ...
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            for row in r.reply_markup.rows:
                for button in row.buttons:
                    if re.search(r"@dirxam_[0-9a-zA-Z]{6}_bot", button.text, re.I):
                        ...
                        self._db.set("dirxam.inline", "bot_token", token)
                        self._token = token
                        ...
                        m = await conv.send_message("DirXamQuery")
                        r = await conv.get_response()
    ...
    async def _dp_revoke_token(self, inited=True) -> None:
        if inited:
            await self._stop()
            logger.error("Ovoz berish mojarosi bor.Tsikinni qaytarib olishga urinish...")

        self._db.set("dirxam.inline", "bot_token", None)
        self._token = None
        if inited:
    ...
    async def _register_manager(
        ...
        try:
            ...
            logger.critical("Token muddati tugagan, bekor qilingan...")
        ...
        try:
            ...
            self.init_complete = False
            logger.critical("Ichki menejerni ishga tushirish muvaffaqiyatsiz tugadi!")
            logger.exception("sababli")
            return False
    ...
    async def _inline_handler(self, inline_query: InlineQuery) -> None:
        ...
        if not query:
            ...
            itext = (
                "😔 inline buyruqlar mavjud emas."
                if not _help
                else f"<b>ℹ️ Mavjud inline buyruqlar:</b>\n\n{_help}"
            )
            ...
            await inline_query.answer(
                [
                    InlineQueryResultArticle(
                        id=rand(20),
                        title="Mavjud inline buyruqlarini ko'rsatish",
        ...
        for mod in self._allmodules.modules:
            ...
            instance = DirXamInlineQuery(inline_query)
                    ...
                    await inline_query.answer(
                        [
                            InlineQueryResultArticle(
                                id=rand(20),
                                title=button["input"],
                                description="⚠️ Iltimos, identifikatorni o'chirmang!",
                                input_message_content=InputTextMessageContent(
                                    "🔄 <b>Userbot-ga qiymatni uzatish...</b>\n"
                                    "<i>Ushbu xabar o'chiriladi...</i>",
                                    "HTML",
                                    disable_web_page_preview=True,
                                ),
                            )
            ...
            for query_func in mod.callback_handlers.values():
                if self.check_inline_security(query_func, query.from_user.id):
                    try:
                        ...
                        await query.answer(
                            "Qayta ishlash so'rovi paytida xatolik yuz berdi.Kirish joylarida ko'proq ma'lumot",
                            show_alert=True,
                        )
                        return
            ...
            for button in array_sum(form.get("buttons", [])):
                if button.get("_callback_data", None) == query.data:
                    if (
                        ...
                        await query.answer("Sizga ushbu tugmani bosishga ruxsat berilmagan!")
                        return
                    ...
                    try:
                        ...
                        logger.exception("Raqamni qaytarish kuzatuvchisiga xato!")
                        await query.answer(
    ...
    async def form(
        ...
        try:
            ...
            msg = (
                "🚫 <b>Muammo inline bot bilan sodir bo'ldi"
                "So'rovni qayta ishlashda. Chiziqlarni tekshirish "
                "further info.</b>"
            )
